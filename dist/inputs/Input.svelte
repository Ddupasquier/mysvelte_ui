...
const updateInputStyle = () => {
  ...
  let additionalStyle = "";
  if (disabled) {
    additionalStyle = `background: #eee; pointer-events: none; padding: ${sizeValues[size]}; ${removeBackgroundStyle(style)};`;
  } else if (isLoading) {
...
.input-container:focus-within .label-in {
  top: -0.25rem;
...
.input-container .options .clear-button {
  display: flex;
  align-items: center;
...
.input-container .options .eye-button {
  display: flex;
  align-items: center;
  justify-content: center;
  pointer-events: all;
